...
//
// Migrations are applied once per schema version to a template database;
// every call to New then clones the template (CREATE DATABASE ... TEMPLATE,
// a few milliseconds) so each test gets a private, fully migrated database it
// can freely commit to — no shared state, no rollback tricks, and
// t.Parallel() is safe. The database is dropped when the test finishes.
//
// Requires the postgres-test compose service (just test starts it):
//
//	docker compose up postgres-test -d
//
// The compose service has no volume, so leftovers from killed test runs are
// wiped by simply restarting it.
package testdb
...
// advisoryLockID serializes template creation and cloning across parallel
// tests and test binaries — Postgres rejects concurrent operations on a
// database that is being used as a clone source.
const advisoryLockID = 987_654_321

// New returns a pool connected to a fresh, fully migrated database owned by
// this test alone.
...
// version doesn't have one yet. Must be called while holding the advisory
// lock. Templates from older schema versions are dropped along the way.
func ensureTemplate(ctx context.Context, conn *pgx.Conn, base string) (string, error) {
	name := "app_test_template_" + postgres.MigrationsHash()
	...
	// Cloning requires zero connections to the template, so close before use.
	pool.Close()
	if err != nil {
		// Don't leave a half-migrated template behind for the next run.
...
// withDatabase swaps the database name in a connection URL. base was already
// used to connect, so it is known to parse.
